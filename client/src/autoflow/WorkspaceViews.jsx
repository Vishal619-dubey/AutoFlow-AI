...
import BioTrustCard from "./BioTrustCard";
import BioTrustDownloadModal from "./BioTrustDownloadModal";
...
const getDownloadErrorPayload = async (error) => {
  try {
...
export function DashboardPage() {
  ...
  return <>
    ...
    <section className="flow-metrics">
      <MetricCard icon={FileCheck2} label="Documents processed" value={loading ? "Ã¢â‚¬â€" : m.processed || 0} detail="AI-classified documents" tone="violet" />
      <MetricCard icon={Workflow} label="Active automations" value={loading ? "Ã¢â‚¬â€" : m.activeRules || 0} detail="Rules currently running" tone="blue" />
      <MetricCard icon={CircleAlert} label="Needs review" value={loading ? "Ã¢â‚¬â€" : m.needsReview || 0} detail="Human decision required" tone="amber" />
      <MetricCard icon={Clock3} label="Time saved" value={loading ? "Ã¢â‚¬â€" : `${m.timeSavedMinutes || 0}m`} detail="Estimated manual effort" tone="green" />
    </section>
    ...
    <section className="flow-grid-main bottom">
      ...
      <div className="flow-card"><div className="flow-card-head"><div><h2>Approval inbox</h2><p>Priority items requiring attention</p></div><Inbox /></div>{data.recentDocuments.filter((d) => d.workflowStatus === "review").slice(0, 3).map((doc) => <div className="flow-inbox-item" key={doc._id}><div className="flow-file-icon"><FileText /></div><div><b>{doc.filename}</b><span>{doc.classification} Ã‚Â· {doc.priority} priority</span></div><ChevronRight /></div>)}{!data.recentDocuments.some((d) => d.workflowStatus === "review") && <div className="flow-empty-mini"><CheckCircle2 /><b>Inbox is clear</b><span>No documents need review.</span></div>}</div>
    </section>
  </>;
}
...
function DocumentTable({
  ...
  if (!documents.length) {
    return (
      <div className="flow-empty">
        ...
        <p>
          Upload a document and AutoFlow will classify
  ...
  const process = async (id) => {
    try {
      await API.post(`/automation/process/${id}`);
  ...
  const performDownload = async (
    ...
    if (proofToken) {
      headers["X-BioTrust-Proof"] =
        proofToken;
    }

    const response = await API.get(
      `/documents/download/${doc._id}`,
      {
        responseType: "blob",
        headers,
      }
  ...
  const downloadDocument = async (doc) => {
    try {
      ...
      if (
        ...
        toast(
          "BioTrust verification required for this protected document."
        );
      ...
      if (
        ...
        toast.error(
          "Enroll your face in AutoFlow BioTrust from Settings before downloading this protected document."
        );
  ...
  const downloadAfterVerification =
    async (proofToken) => {
      if (!pendingDownload) {
        return false;
      }
      ...
      try {
        ...
        toast.success(
          "Identity verified. Secure download started."
        );
        ...
        console.error(
          "BioTrust secure download error:",
          error
        );
  ...
  const removeDocument = async (doc) => {
    if (
      !window.confirm(
        `Move "${doc.filename}" to Trash? You can restore it later.`
      )
    ) {
      return;
    }
                  ...
                  <td>
                    <div className="flow-row-actions">
                      {doc.fileType ===
                        "pdf" && (
                        <button
                          className="flow-icon-btn evidence"
                      ...
                      <button
                        className="flow-icon-btn report"
...
export function TrashPage() {
  ...
  return <><PageHeader eyebrow="DOCUMENT RETENTION" title="Trash" description="Restore accidentally removed documents or permanently erase files you no longer need." action={documents.length > 0 ? <span className="flow-trash-count">{documents.length} deleted documents</span> : null} /><div className="flow-card"><div className="flow-table-wrap"><table className="flow-table flow-trash-table"><thead><tr><th>Document</th><th>Classification</th><th>Priority</th><th>Deleted</th><th>Actions</th></tr></thead><tbody>{documents.map((doc) => <tr key={doc._id}><td><div className="flow-doc-name"><span><FileText /></span><div><b>{doc.filename}</b><small>{formatSize(doc.filesize)} Ã‚Â· {doc.fileType?.toUpperCase()}</small></div></div></td><td>{doc.classification || "General"}</td><td>{badge(doc.priority || "medium")}</td><td>{new Date(doc.updatedAt).toLocaleString()}</td><td><div className="flow-row-actions"><button className="flow-restore-btn" onClick={() => restore(doc)}><RotateCcw /> Restore</button><button className="flow-delete-forever" onClick={() => removeForever(doc)}><Trash2 /> Delete forever</button></div></td></tr>)}</tbody></table></div>{!loading && !documents.length && <div className="flow-empty"><Trash2 /><h3>Trash is empty</h3><p>Documents moved to Trash will appear here until permanently deleted.</p></div>}</div></>;
}
...
export function AuditPage() {
  ...
  return <>
    ...
    <section className="flow-metrics">
      <MetricCard icon={Activity} label="Total executions" value={loading ? "Ã¢â‚¬â€" : runs.length} detail="Recorded automation runs" tone="violet" />
      <MetricCard icon={CheckCircle2} label="Successful" value={loading ? "Ã¢â‚¬â€" : runs.filter((run) => run.status === "success").length} detail="Completed without errors" tone="green" />
      <MetricCard icon={CircleAlert} label="Failed" value={loading ? "Ã¢â‚¬â€" : runs.filter((run) => run.status === "failed").length} detail="Runs needing attention" tone="amber" />
...
export function SecurityPage() {
  ...
  const verify = async (doc) => {
    try {
      ...
      toast.success(`${endpoint === "protect" ? "Encrypted and verified" : "Integrity verified"} Ã‚Â· Trust ${result.trust.score}/100`);
      await load();
    } catch (error) {
  ...
  return <>
    ...
    <section className="flow-metrics">
      <MetricCard icon={ShieldCheck} label="Encrypted" value={loading ? "Ã¢â‚¬â€" : metrics.encryptedDocuments || 0} detail="AES-256-GCM at rest" tone="blue" />
      <MetricCard icon={ShieldCheck} label="Integrity verified" value={loading ? "Ã¢â‚¬â€" : metrics.verifiedDocuments || 0} detail="SHA-256 fingerprints" tone="green" />
      <MetricCard icon={ShieldAlert} label="Restricted" value={loading ? "Ã¢â‚¬â€" : metrics.restrictedDocuments || 0} detail="Trust policy blocked" tone="amber" />
      <MetricCard icon={Search} label="PII findings" value={loading ? "Ã¢â‚¬â€" : metrics.totalFindings || 0} detail="Masked privacy matches" tone="violet" />
    </section>
    ...
    <div className="flow-card"><div className="flow-card-head"><div><h2>Document trust register</h2><p>Verify cryptographic integrity and review privacy or prompt-injection risk.</p></div></div>
      <div className="flow-table-wrap"><table className="flow-table flow-security-table"><thead><tr><th>Document</th><th>Encryption</th><th>Integrity</th><th>Trust score</th><th>Privacy risk</th><th>Actions</th></tr></thead><tbody>{data.documents.map((doc) => {
        ...
        return <tr key={doc._id}><td><div className="flow-doc-name"><span><FileText /></span><div><b>{doc.filename}</b><small>{doc.classification || "General"} Ã‚Â· {doc.fileType?.toUpperCase()}</small></div></div></td><td>{badge(security.encryption || "Legacy")}</td><td>{badge(security.integrityStatus || "pending")}</td><td><div className="flow-risk-score"><i style={{ width: `${security.trustScore || 0}%` }} /><span>{security.trustScore || 0}/100 Ã‚Â· {security.trustGrade || "restricted"}</span></div></td><td>{scanned ? badge(result.riskLevel || "safe") : badge("Not scanned")}</td><td><div style={{display:"flex",gap:8}}><button type="button" className="flow-icon-btn" title={security.encryption === "AES-256-GCM" ? "Verify integrity" : "Encrypt and protect legacy file"} onClick={() => verify(doc)} disabled={verifying === doc._id}><ShieldCheck className={verifying === doc._id ? "flow-spin" : ""} /></button><button type="button" className="flow-icon-btn" title="Scan sensitive data" onClick={() => scan(doc._id)} disabled={scanning === doc._id}><RefreshCw className={scanning === doc._id ? "flow-spin" : ""} /></button></div></td></tr>;
      })}</tbody></table></div>
...
export function ExecutiveReportPage() {
  ...
  return <div className="flow-report-shell"><div className="flow-report-actions"><button className="flow-ghost" onClick={() => navigate("/documents")}><ArrowLeft size={16} /> Back to documents</button><button className="flow-primary" onClick={() => window.print()}><Printer size={16} /> Download / Print PDF</button></div><article className="flow-report-page">
    ...
    <section className="flow-report-meta"><div><span>Classification</span><b>{doc.classification}</b></div><div><span>Priority</span><b className={`tone-${doc.priority}`}>{doc.priority}</b></div><div><span>Pages</span><b>{doc.pages || "Ã¢â‚¬â€"}</b></div><div><span>Word count</span><b>{report.wordCount.toLocaleString()}</b></div><div><span>AI confidence</span><b>{doc.automationScore}%</b></div><div><span>Workflow</span><b>{doc.workflowStatus}</b></div></section>
    ...
    <section className="flow-report-assurance"><div><ShieldCheck /><span><small>PRIVACY RISK</small><b>{privacy.riskScore}/100 Ã‚Â· {privacy.riskLevel}</b><p>{privacy.totalFindings} sensitive findings detected</p></span></div><div><Fingerprint /><span><small>DOCUMENT INTEGRITY</small><b>SHA-256 verified</b><p>{integrity.fingerprint.slice(0, 24)}...</p></span></div><div><Gauge /><span><small>AUTOMATION QUALITY</small><b>{doc.automationScore}% confidence</b><p>Evidence traceability enabled</p></span></div></section>
